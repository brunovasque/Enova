...
function classifyEnvioDocsBasicValidation({ mediaObject, normalizedMsg, target, hintText }) {
  ...
  if (sourceType === "image") {
  if (hasKnownFileSize && fileSize < MIN_IMAGE_SIZE_BYTES) {
...
function buildCorrespondentePrivateWhatsAppMessage({ payloadTecnico, dossieCanonico }) {
  const meta = dossieCanonico?.meta && typeof dossieCanonico.meta === "object" ? dossieCanonico.meta : {};
  const caseRef = String(meta?.case_ref || meta?.pre_cadastro_numero || "000000").trim();
  return [
...
async function handleCorrespondenteEntryPage(request, env) {
  ...
  const postForm = request.method === "POST" ? await request.formData().catch(() => null) : null;
  const caseRefInput = (postForm ? String(postForm.get("pre") || "") : "")
  ...
  const tokenInput = (postForm ? String(postForm.get("t") || postForm.get("token") || "") : "")
...
function buildCorrespondentePrivateDeliveryMessage(caseRefInput, entryLink) {
  const caseRef = normalizeCorrespondenteCaseRefInput(caseRefInput) || "000000";
  const linkSafe = String(entryLink || "").trim();
  return [
    "✅ Caso assumido com exclusividade.",
    `Pré-cadastro: ${caseRef}`,
    linkSafe
...
async function sendCorrespondentePrivateDelivery(env, correspondenteWaId, token, caseRefInput) {
  const tokenSafe = normalizeAssumirToken(token) || "";
  const entryLink = buildCorrespondenteEntryLinkForCorrespondente(env, tokenSafe, correspondenteWaId, caseRefInput);
  const privateMessage = buildCorrespondentePrivateDeliveryMessage(caseRefInput, entryLink);
  const payloads = splitMessageForWhatsapp(privateMessage);
...
async function listOperationalCasesForSenderFallback(env, limit = 5) {
  const projection = "wa_id,nome,fase_conversa,pre_cadastro_numero,corr_publicacao_status,corr_lock_correspondente_wa_id,processo_enviado_correspondente,aguardando_retorno_correspondente,updated_at";
...
async function handleCorrespondenteReturnByCaseRef(env, msg, userText) {
  ...
  const askCaseConfirmation = async (reason, options = {}) => {
  ...
  const emitStatusDecisionProbe = async ({ statusClassificado, handled, fallbackCommonFlow, decisionReason, nextStage = null }) => {
    await telemetry(env, {
      ...
      details: {
        ...
        status_raw: statusProbe.statusRaw || null,
        status_normalized: statusProbe.statusNormalized || null,
        status_classificado: statusClassificado || null,
        handled: handled ? "sim" : "nao",
        fallback_common_flow: fallbackCommonFlow ? "sim" : "nao",
  ...
  const emitSenderGateProbe = async ({
    ...
    await telemetry(env, {
      wa_id: correspondenteWaId,
      event: "corr_sender_gate_probe",
      stage: "aguardando_retorno_correspondente",
      severity: "info",
      force: true,
      message: "Probe objetiva: gate de remetente para retorno do correspondente",
      details: {
        case_ref: caseRef || null,
        from_wa_id_raw: correspondenteWaId || null,
        from_wa_id_cmp: fromWaIdCmp || null,
  ...
  const fallbackStatusClassificado = normalizeCorrespondenteReturnStatus(classifyCorrespondenteReturnFromText(consolidatedText));
  const preliminaryStatusClassificado =
    fastPathStatusClassificado ||
    (fallbackStatusClassificado !== "nao_identificado" && fallbackStatusClassificado !== "em_analise"
  ...
  if (!lockAtualCmp) {
    ...
    const senderFallbackCasesBase = senderLockedCases.length
      ? senderLockedCases
      : (isCorrespondenteChannel ? await listOperationalCasesForSenderFallback(env, 6) : []);
    const senderFallbackCandidates = [...senderFallbackCasesBase];
    if (senderLockedCases.length > 0 && waCliente) {
      const alreadyIncluded = senderFallbackCandidates.some((row) => String(row?.wa_id || "").trim() === waCliente);
      if (!alreadyIncluded) {
        senderFallbackCandidates.push({ wa_id: waCliente });
      }
    }
    const hasUniqueSenderCompatibleCase = senderFallbackCandidates.length === 1;
    ...
    await emitSenderGateProbe({
      ...
      decisionReason: hasUniqueSenderCompatibleCase
        ? "case_lock_missing_unique_sender_case_fallback"
        : (hasSenderAmbiguity
  ...
  await emitStatusDecisionProbe({
    statusClassificado: statusCanonico,
    handled: true,
    fallbackCommonFlow: false,
    decisionReason,
...
async function handleCorrespondenteAssumirCommand(env, msg, userText) {
  ...
  const tokenResolved = normalizeAssumirToken(token || stCaso?.corr_assumir_token || "");
  ...
  try {
    ...
    await sendCorrespondentePrivateDelivery(env, correspondenteWaIdOutbound, tokenResolved, caseRef);
    await telemetry(env, {
      wa_id: caso.wa_id,
