...
function buildGateFinaisGuidance(request) {
  ...
  // BLOCO 9 — regularizacao_restricao (titular): contrato exige pergunta sobre POSSIBILIDADE de regularizar (não status)
  if (stage === "regularizacao_restricao") {
    if (/\bestou\s*negociando\b|\bem\s*negociacao\b|\bem\s*negociação\b|\bnegociando\b|\bnegociacao\b|\bnegociação\b/.test(normalizedMessage)) {
      return "Entendido, negociação é um passo. Você tem possibilidade real de regularizar a restrição no seu CPF? Responda *sim*, *não* ou *não sei*.";
    }
    if (/\bjá\s*quitei\b|\bjá\s*paguei\b|\bquitei\b|\bpaguei\b|\bpagamento\s*feito\b/.test(normalizedMessage)) {
      return "Ótimo, pagamento feito é importante. Você tem possibilidade de regularizar essa restrição no CPF? Responda *sim*, *não* ou *não sei*.";
    }
    if (/\bainda\s*nao\s*baixou\b|\bainda\s*não\s*baixou\b|\bnao\s*baixou\b|\bnão\s*baixou\b|\bpendente\s*no\s*cpf\b/.test(normalizedMessage)) {
      return "Entendido. Você tem possibilidade de regularizar essa pendência no CPF? Responda *sim*, *não* ou *não sei*.";
    }
    if (/\bisso\s*já\s*serve\b|\bjá\s*serve\b|\bjá\s*conta\b|\bjá\s*basta\b/.test(normalizedMessage)) {
      return "O sistema verifica a situação completa. Você tem possibilidade real de regularizar a restrição? Responda *sim*, *não* ou *não sei*.";
    }
    if (/\bvou\s*ver\b|\bvamos\s*ver\b|\btalvez\b|\best[aá]\s*tentando\b|\bnao\s*sei\b|\bnão\s*sei\b|\bnao\s*sabe\b|\bnão\s*sabe\b/.test(normalizedMessage)) {
      return "Entendido. Me confirma: você tem possibilidade de regularizar a restrição no seu CPF? Responda *sim*, *não* ou *não sei*.";
    }
    return "Você tem possibilidade de regularizar a restrição no seu CPF? Responda *sim*, *não* ou *não sei*.";
  }
...
function buildTopoFunilGuidance(request) {
  ...
  if (stage === "inicio_programa") {
    ...
    if (/(?:como funciona|explic[aeo]|explique|me explic[aeo]|me explique|n[aã]o.*me explic[aeo]|n[aã]o.*me explique|como [eé]|que [eé] isso|como que funciona|funciona como)/i.test(normalizedMessage)) {
