...
## Regras absolutas
1) 1 tarefa = 1 PR = 1 branch até terminar
2) Nunca misturar escopos no mesmo PR (Worker OU Panel OU Workflows OU Docs)
3) Toda mudança precisa de commit real (hash obrigatório)
4) Sem hash = não feito
5) Resposta final deve incluir: PR#, branch, commit hash, arquivos alterados, smoke tests, rollback
...
Roteiro de trabalho (padrão Codex — Enova)
...
plano de rollback (ex.: git revert <hash>)
...
Trabalhe na PR #<NUMERO> e no mesmo branch dela (não crie PR nova).

Ajuste solicitado
<descreva o ajuste objetivo>
Obrigatório
...
Se você NÃO tem permissão, diga explicitamente: “não consigo commitar/push neste repo/branch” e explique o motivo (ex.: permissões, branch protegido, modo read-only).
Sem hash = não feito.
