# FUNIL COPY EXTRACT V2

## GAPS & REGRAS CANÔNICAS (pendentes de patch runtime)

### Gap 1 — Parceiro(a) aposentado(a)
- Regra canônica: considerar renda de parceiro(a) aposentado(a) normalmente na composição.
- Se a composição com parceiro(a) inviabilizar por idade/regras, oferecer rota segura:
  - seguir só 1 pessoa sem reclassificar estado civil; ou
  - compor renda com parente próximo.
- Stages impactados (aprox.):
  - `estado_civil` (anchor: case 'estado_civil', aprox. linha 4524)
  - `financiamento_conjunto` (anchor: case 'financiamento_conjunto', aprox. linha 4866)
  - `parceiro_tem_renda` (anchor: case 'parceiro_tem_renda', aprox. linha 5008)
  - `regime_trabalho_parceiro` (anchor: case 'regime_trabalho_parceiro', aprox. linha 6391)
  - `renda_parceiro` (anchor: case 'renda_parceiro', aprox. linha 6635)
  - `quem_pode_somar` (anchor: case 'quem_pode_somar', aprox. linha 7184)

### Gap 2 — MEI
- Regra canônica: tratar MEI como autônomo (PF) para fins de enquadramento no funil.
- IR válido para esta análise: PF. IR PJ não vale para MCMV.
- Stages impactados (aprox.):
  - `regime_trabalho` (anchor: case 'regime_trabalho', aprox. linha 5967)
  - `inicio_multi_regime_pergunta` (anchor: case 'inicio_multi_regime_pergunta', aprox. linhas 6142/6276)
  - `inicio_multi_regime_coletar` (anchor: case 'inicio_multi_regime_coletar', aprox. linhas 6204/6339)
  - `ir_declarado` (anchor: case 'ir_declarado', aprox. linha 7417)
  - `autonomo_compor_renda` (anchor: case 'autonomo_compor_renda', aprox. linha 7585)
...
- options:
  - sim (já sabe como funciona o programa)
  - não (quer explicação rápida antes)
- fallback:
...
- options:
  - nome completo (ex.: Bruno Vasques)
  - estado civil: solteiro(a), casado(a), união estável, separado(a), divorciado(a) ou viúvo(a)
- fallback:
...
- options:
  - (manual_required)
  - suggested: válido (com validade definida), indeterminado
- fallback:
...
- options:
  - estado civil: solteiro(a), casado(a), união estável, separado(a), divorciado(a), viúvo(a)
  - composição de renda (quando aplicável): só sua renda, parceiro(a), familiar
  - casamento/relação (quando aplicável): civil no papel, união estável
  - financiamento (quando aplicável): comprar juntos, só você, apenas se precisar
- fallback:
...
- options:
  - casamento/relação: civil no papel, união estável
  - compra: juntos, só você, apenas se precisar
  - tipo de trabalho: CLT, autônomo(a), servidor(a)
- fallback:
  - (não identificado)
- interruptor: yes
- anchors:
...
- options:
  - parceiro(a) com renda: tem renda, não tem renda
  - tipo de trabalho (quando houver renda): CLT, autônomo(a), servidor(a)
- fallback:
  - (não identificado)
- interruptor: yes
- anchors:
...
- options:
  - composição: só sua renda, parceiro(a), familiar
  - parceiro(a): tem renda própria, não tem renda
  - familiar (exemplos do texto): pai, mãe, irmão(ã), avô(ó), tio(a)
- fallback:
  - (não identificado)
- interruptor: yes
- anchors:
...
- main:
  ...
  - Entendi! 👌 | Só me confirma uma coisinha… | *Seu avô/avó recebe aposentadoria ou outro tipo de benefício?* (aprox. linha 5379)
...
- options:
  - tipo de familiar: pai, mãe, irmão(ã), avô(ó), tio(a), primo(a)
  - regime de trabalho do familiar: CLT/carteira assinada, autônomo(a), servidor(a)
  - benefício do avô/avó: aposentadoria, outro tipo de benefício
- fallback:
  - (não identificado)
- interruptor: yes
- anchors:
...
- main:
  - Perfeito 👌 | Então vamos considerar a renda da aposentadoria. | Agora me fala: esse familiar é *CLT*, *autônomo(a)* ou *servidor(a)*? Ou só recebe o benefício? (aprox. linha 5579)
  - Perfeito! 👍 | Então vamos considerar a aposentadoria. | E sobre atividade atual… esse familiar trabalha (CLT/autônomo/servidor) ou só recebe o benefício? (aprox. linha 5611)
  ...
  - Consegue me confirmar qual é o tipo de benefício *do seu avô/avó*? | Pode ser: aposentadoria, pensão, BPC/LOAS ou outro benefício 👍 (aprox. linha 5702)
- options:
  - tipo de benefício: aposentadoria, pensão, BPC/LOAS, outro benefício
  - atividade atual: CLT, autônomo(a), servidor(a), só recebe benefício
- fallback:
  - (não identificado)
- interruptor: yes
- anchors:
...
- options:
  - renda mensal da pessoa que vai compor (valor numérico em reais)
- fallback:
...
- options:
  ...
  - formato de coleta quando sim: tipo — valor bruto (ex.: Bico — 1200)
- fallback:
...
- options:
  - renda mensal aproximada (valor em reais)
  - renda extra: sim ou não
- fallback:
...
- options:
  - renda mensal do parceiro(a) (valor em reais)
  - Imposto de Renda: sim ou não
  - CTPS 36 meses: sim ou não
- fallback:
...
- options:
  - renda mensal do familiar/parceiro(a) para composição (valor em reais)
  - Imposto de Renda: sim ou não
- fallback:
  - (não identificado)
- interruptor: yes
...
- options:
  - formato: valor regime + valor regime (ex.: 2000 CLT + 1200 Uber)
- fallback:
  - (não identificado)
- interruptor: yes
- anchors:
...
- options:
  - sim (informa valor e tipo da renda extra)
  - não (segue sem renda extra)
- fallback:
...
- options:
  - comprova renda autônoma: sim, não
  - composição alternativa: parceiro(a), familiar, sozinho(a)
- fallback:
...
- options:
  - CTPS 36 meses: sim, não
  - status aproximado: próximo, bem distante
  - dependente menor de 18: sim, não
...
- options:
  - parceiro(a) com 36 meses de CTPS: sim, não
  - status aproximado: próximo, bem distante
  - dependente menor de 18: sim, não
  - restrição no CPF: sim, não
- fallback:
...
- options:
  - tem dependente menor de 18: sim, não
  - tem restrição no CPF: sim, não
- fallback:
...
- options:
  - está regularizando: sim, não
  - acha que pode ter pendência: sim, não
  - receber lista de documentos: sim, não
- fallback:
...
- options:
  - quer receber instrução rápida + lista de documentos: sim, não
- fallback:
...
- options:
  - quer receber lista de documentos: sim, não
  - envio aceito: foto, PDF, áudio
- fallback:
  - (não identificado)
- interruptor: yes
- anchors:
