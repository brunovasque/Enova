/**
 * cognitive_topo_harmonization_pr3.smoke.mjs
 *
 * Smoke tests for PR3 — Gap B5 fix: "começar do zero" → reset em inicio_decisao.
 *
 * Esta suite estende PR2 (18 casos) com 9 novos casos (total: 27).
 * O gap B5 é a correção principal: "começar do zero" sem "quero" prefixo.
 *
 * Validates:
 * 1–18. Regressão: todos os 18 casos do PR2 preservados
 * 19.  B5 FIX: inicio_decisao "começar do zero" → reset (era fallback/loop antes da correção)
 * 20.  inicio_decisao: "comecar do zero" (sem acento) → reset
 * 21.  inicio_decisao: "quero começar do zero" → reset (preservado)
 * 22.  inicio_decisao: "quero comecar do zero" → reset (preservado)
 * 23.  inicio_decisao: "prefiro recomeçar" → reset (preservado)
 * 24.  inicio_decisao: "do zero" → reset (exato, preservado)
 * 25.  inicio_decisao: "2" → reset (preservado)
 * 26.  inicio_decisao: "1" → continuar (preservado — sem regressão)
 * 27.  inicio_decisao: "oi" → fallback (sem match em opcao1 ou opcao2)
 */
...
// ── Parser mirrors from worker (fixed — inclui "comecar do zero|começar do zero") ──
...
console.log("\n🔬 TOPO HARMONIZATION PR3 — Smoke Tests (27 casos)\n");
console.log("📌 Gap B5: 'começar do zero' deve ser reset em inicio_decisao\n");

// ================================================================
// REGRESSÃO PR2 (casos 1–18)
// ================================================================
console.log("── Regressão PR2 (casos 1–18) ──");
...
// ===== 7. inicio_rnm fallback aligns with parser =====
...
// ===== 8. inicio_rnm_validade fallback aligns with parser =====
...
// ===== 9. reanchor-helper: all 7 topo stages map to "topo" =====
...
// ===== 10. STAGE_CONTRACT_METADATA: inicio_nacionalidade canonical_prompt =====
await asyncTest('10. inicio_nacionalidade canonical_prompt says "brasileiro/estrangeiro"', async () => {
...
// ================================================================
// NOVOS CASOS PR3 — Gap B5 e regressão ampliada (casos 19–27)
// ================================================================
console.log("\n── Novos casos PR3 (B5 fix + regressão ampliada, casos 19–27) ──");

// ===== 19. B5 FIX: "começar do zero" → reset (era fallback/loop antes) =====
await asyncTest('19. [B5 FIX] inicio_decisao: "começar do zero" → reset', async () => {
  assert.equal(
    parseInicioDecisao("começar do zero"),
    "reset",
    '"começar do zero" must resolve to reset, not fallback'
  );
});

// ===== 20. "comecar do zero" (sem acento) → reset =====
await asyncTest('20. inicio_decisao: "comecar do zero" (sem acento) → reset', async () => {
  assert.equal(parseInicioDecisao("comecar do zero"), "reset");
});

// ===== 21. "quero começar do zero" → reset (preservado) =====
await asyncTest('21. inicio_decisao: "quero começar do zero" → reset (preservado)', async () => {
  assert.equal(parseInicioDecisao("quero começar do zero"), "reset");
});

// ===== 22. "quero comecar do zero" (sem acento) → reset (preservado) =====
await asyncTest('22. inicio_decisao: "quero comecar do zero" (sem acento) → reset (preservado)', async () => {
  assert.equal(parseInicioDecisao("quero comecar do zero"), "reset");
});

// ===== 23. "prefiro recomeçar" → reset (preservado) =====
await asyncTest('23. inicio_decisao: "prefiro recomeçar" → reset (preservado, regressão)', async () => {
  assert.equal(parseInicioDecisao("prefiro recomeçar"), "reset");
});

// ===== 24. "do zero" (exato) → reset (preservado) =====
await asyncTest('24. inicio_decisao: "do zero" (exato) → reset (preservado)', async () => {
  assert.equal(parseInicioDecisao("do zero"), "reset");
});

// ===== 25. "2" → reset (preservado) =====
await asyncTest('25. inicio_decisao: "2" → reset (preservado, regressão)', async () => {
  assert.equal(parseInicioDecisao("2"), "reset");
});

// ===== 26. "1" → continuar (sem regressão) =====
await asyncTest('26. inicio_decisao: "1" → continuar (sem regressão após fix)', async () => {
  assert.equal(parseInicioDecisao("1"), "continuar");
});

// ===== 27. "oi" → fallback (sem match — não virou reset) =====
await asyncTest('27. inicio_decisao: "oi" → fallback (sem colapso indevido)', async () => {
  assert.equal(parseInicioDecisao("oi"), "fallback");
});
...
if (failed > 0) {
  console.error("\n❌ TOPO HARMONIZATION PR3 — Some tests FAILED");
  process.exit(1);
} else {
  console.log("\n✅ TOPO HARMONIZATION PR3 — All 27 tests passed");
}
