...
// 3.8i) Fonte única web: restrição final do state prevalece sobre pacote legado.
{
  const env = buildEnvWithState();
  const st = env.__enovaSimulationCtx.stateByWaId[waCaso];
  st.corr_lock_correspondente_wa_id = correspondenteWa;
  ...
  st.corr_publicacao_status = "entregue_privado_aguardando_retorno";
  st.restricao = "sim";
  st.pacote_restricoes_json = { resumo: "sem_restricao", participantes: [{ participante: "p1", tem_restricao: false }] };
  const req = new Request(`https://worker.local/correspondente/entrada?pre=000001&cw=${correspondenteWa}`, { method: "GET" });
  const res = await worker.fetch(req, env, {});
  const body = await res.text();
  assert.equal(res.status, 200);
...
// 3.8j) Fonte única web: doc persistido em enova_docs deve vencer pendência do checklist/state.
{
  ...
  env.__enovaSimulationCtx.docsByWaId = {
    [waCaso]: [
      { doc_id: "doc-ctps-001", tipo: "ctps_completa", participante: "p1", status: "recebido", url: "https://docs.example.com/ctps-real.pdf" }
    ]
  };
  const req = new Request(`https://worker.local/correspondente/entrada?pre=000001&cw=${correspondenteWa}`, { method: "GET" });
  const res = await worker.fetch(req, env, {});
...
// 27) lock salvo sem 55 + retorno com 55 deve casar no gate e seguir operacional.
{
  const env = buildEnvWithState();
  env.__enovaSimulationCtx.stateByWaId[waCaso].pre_cadastro_numero = "000518";
  const originalConsoleLog = console.log;
...
// 28) lock salvo com 55 + retorno sem 55 deve casar no gate e seguir operacional.
{
  const env = buildEnvWithState();
  env.__enovaSimulationCtx.stateByWaId[waCaso].pre_cadastro_numero = "000519";
  env.__enovaSimulationCtx.stateByWaId[waCaso].corr_assumir_token = "CD34EF56GH78JK90LM12NO34";
  env.__enovaSimulationCtx.stateByWaId[waCaso].corr_publicacao_status = "publicado_grupo_pendente_assumir";
        ...
        entry: [{
          changes: [{
            value: {
              messages: [{
                from: "5541999997777",
                id: "wamid.assumir.readonly.waid.probe.v2",
                timestamp: "1773183962",
                type: "text",
                text: { body: `ASSUMIR ${tokenAlt}` }
              }],
              contacts: [{ wa_id: "5541999997777" }],
              metadata: { phone_number_id: "test" }
            }
          }]
        }]
        ...
        entry: [{
          changes: [{
            value: {
              messages: [{
                from: "41999997777",
                id: "wamid.retorno.readonly.waid.probe.v2",
                timestamp: "1773183963",
                type: "text",
                text: { body: "Pré-cadastro #000519\nSTATUS: APROVADO" }
              }],
              contacts: [{ wa_id: "41999997777" }],
              metadata: { phone_number_id: "test" }
            }
          }]
        }]
